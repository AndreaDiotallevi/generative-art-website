...
const Artwork = props => {
  ...
  const artwork = artworks.find(artwork => artwork.title === title.replace('-', ' '));

  function renderImages() {
    let images = [];
    for (let i = 0; i < artwork.variations; i++) {
      images.push(<img key={`${title}-${i}`} id={`artwork-page-image-${i}`} className='artwork-page-image' src={require(`../../assets/${title}-${i}.png`)} alt={title}></img>)
    }
    return images;
  }
